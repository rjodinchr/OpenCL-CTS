...
def categorize_difference(cmd, sub_test, expected, actual):
    ...
    if actual == "pass":
        return f"🎉 FIX: [{cmd}] '{sub_test}' expected '{expected}', but got '{actual}'."
    elif actual == "fail":
        return f"❌ REGRESSION: [{cmd}] '{sub_test}' expected '{expected}', but got '{actual}'."
    else:
        return f"⚠️ DIFFERENCE: [{cmd}] '{sub_test}' expected '{expected}', but got '{actual}'."
...
def print_report_and_exit(has_error, differences, missing_refs):
    ...
    if has_error:
        print("\n❌ Errors found during comparison. Failing the check.")
        sys.exit(1)
    else:
...
def main():
    ...
    if not results_dir.is_dir():
        print(f"Error: Results directory '{args.results_dir}' does not exist or is empty.")
        sys.exit(1)
